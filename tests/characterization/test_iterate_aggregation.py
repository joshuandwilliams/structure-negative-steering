...
def test_aggregation_collapses_real_seeds_into_one_row():
    ...
    agg = its._aggregate_per_sequence(rows, "steered_")
...
def test_aggregating_no_rows_does_not_raise():
    agg = its._aggregate_per_sequence([], "steered_")
...
def test_majority_positions_keeps_what_most_seeds_agree_on():
    ...
    positions, counts, n_seeds = its._agg_majority_positions([[5, 7], [5, 7], [5]])
...
def test_a_minority_position_is_dropped():
    positions, counts, _ = its._agg_majority_positions([[5], [5], [9]])
...
def test_majority_positions_of_nothing_is_empty():
    positions, counts, n_seeds = its._agg_majority_positions([])
...
def test_unified_ranks_order_by_the_composite():
    ...
    its._compute_unified_ranks(rows)
...
def test_ranking_metric_reads_the_named_column(name):
    got = its._ranking_metric({name: "3.5"}, name)
...
def test_ranking_metric_of_a_missing_column_is_none_not_zero():
    ...
    assert its._ranking_metric({}, "ra_eff_vs_truth") in (None, float("inf")) or \
        its._ranking_metric({}, "ra_eff_vs_truth") != 0
...
def test_outcome_classification_returns_a_label_and_a_reason():
    ...
    label, reason = its._classify_outcome(
...
def test_no_reverted_prediction_is_reported_as_such():
    ...
    label, reason = its._classify_outcome({}, {}, 5.0, [])
...
def test_verdict_breakdown_counts_each_class():
    ...
    got = its._per_seed_verdict_breakdown(rows, 5.0)
...
def test_pose_holds_and_clean_steered_are_recognised():
    ...
    assert callable(its._row_is_pose_holds)
    assert callable(its._row_is_clean_steered)
    assert its._row_is_pose_holds({}) in (True, False)
    assert its._row_is_clean_steered({}) in (True, False)
...
def test_resume_key_is_the_prediction_path():
    ...
    assert its._resume_key_for_row(row) == its._resume_key_for_row(dict(row))


@pytest.mark.local_unit
def test_resume_key_distinguishes_two_seeds_of_one_design():
    a = its._resume_key_for_row({"pdb": "/r/steered/design_03_s0/prediction.pdb"})
    b = its._resume_key_for_row({"pdb": "/r/steered/design_03_s1/prediction.pdb"})
...
def test_confidence_flag_is_ok_when_every_threshold_is_met():
    row = {"steered_ipsae_min": "0.5", "steered_pae_pass_frac": "0.9",
           ...
           "steered_ipae": "5.0"}
    assert its._classify_steered_confidence_flag(row, 0.5, 0.4, 0.7, 10.0) == "ok"
...
def test_confidence_flag_names_the_single_failing_threshold():
    row = {"steered_ipsae_min": "0.5", "steered_pae_pass_frac": "0.1",
           "steered_iptm": "0.8", "steered_complex_plddt": "0.9",
           "steered_ipae": "5.0"}
    assert its._classify_steered_confidence_flag(row, 0.5, 0.4, 0.7, 10.0) == \
        "low_pass_frac"
...
def test_confidence_flag_reports_multiple_failures_distinctly():
    row = {"steered_ipsae_min": "0.5", "steered_pae_pass_frac": "0.1",
           ...
           "steered_ipae": "30.0"}
    got = its._classify_steered_confidence_flag(row, 0.5, 0.4, 0.7, 10.0)
...
def test_confidence_flag_is_blank_when_there_are_no_metrics():
    ...
    assert its._classify_steered_confidence_flag({}, 0.5, 0.4, 0.7, 10.0) == ""
...
def test_each_threshold_has_its_own_trigger_name(key, value, expected):
    row = {"steered_ipsae_min": "0.5", "steered_pae_pass_frac": "0.9",
           ...
           "steered_ipae": "5.0"}
    row[key] = value
    assert its._classify_steered_confidence_flag(row, 0.5, 0.4, 0.7, 10.0) == expected
...
def test_aggregate_row_translation_keeps_the_identifiers():
    rows = _per_seed("6G10")
    got = its._translate_aggregate_row(rows[0])
    assert isinstance(got, dict) and got


@pytest.mark.local_unit
def test_translating_an_empty_row_does_not_raise():
    assert isinstance(its._translate_aggregate_row({}), dict)
